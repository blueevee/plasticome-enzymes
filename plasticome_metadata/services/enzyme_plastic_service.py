from plasticome_metadata.models.metadata_enzyme_model import (
    EnzymePlasticTypes,
    PlasticTypes,
    EnzymeMetadata,
)
...
def add_plastic_type_to_enzyme(enzyme: str, plastic_type: str):
    try:
        ...
        saved_instance = EnzymePlasticTypes.create(
            enzyme_id=enzyme, plastic_id=plastic_type
        )
...
def get_all_plastic_types_enzyme():
    try:
        all_enzymes = (
            EnzymePlasticTypes.select(
                EnzymePlasticTypes.id,
                EnzymeMetadata.fungi_name,
            ...
            .join(
                EnzymeMetadata,
                on=(EnzymePlasticTypes.enzyme_id == EnzymeMetadata.id),
            )
            .join(
                PlasticTypes,
                on=(EnzymePlasticTypes.plastic_id == PlasticTypes.id),
